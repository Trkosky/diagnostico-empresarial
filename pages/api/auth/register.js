  ...
  await resend.emails.send({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:0 auto;color:#1e293b">
        ...
        <p>Olá! Seu acesso ao painel de Diagnóstico Empresarial foi criado. Use as credenciais abaixo para entrar:</p>
        <div style="background:#f1f5f9;padding:16px;border-radius:8px;margin:16px 0;border-left:4px solid #1d4ed8">
          <p style="margin:6px 0"><strong>Link de acesso:</strong><br><a href="${loginUrl}" style="color:#1d4ed8">${loginUrl}</a></p>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await sendWelcomeEmail(email, password, mustChangePassword, createdByName)
